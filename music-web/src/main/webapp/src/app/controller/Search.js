...
angular.module("music").controller("Search", function ($rootScope, $scope, $stateParams, Restangular, Playlist) {
  ...
  $rootScope.searchLastFm = function (query) {
    $scope.results = {};
    Restangular.one("search/lastfm")
      .get({ query: query })
      .then(function (data) {
        $scope.results.lastFm = data?.results?.trackmatches?.track;
        console.log($scope.results);
      });
  };

  $rootScope.searchSpotify = function (query) {
    $scope.results = {};
    Restangular.one("search/spotify")
      .get({ query: query })
      .then(function (data) {
        $scope.results.spotify = data?.tracks?.items;
        console.log(data);
      });
  };
});
