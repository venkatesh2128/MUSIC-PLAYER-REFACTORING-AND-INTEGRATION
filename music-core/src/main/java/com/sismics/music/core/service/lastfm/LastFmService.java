...
public class LastFmService extends AbstractScheduledService {
    ...
    public void scrobbleTrackList(User user, List<Track> trackList, List<Date> dateList) {
        ...
        for (int i = 0; i < trackList.size(); i++) {
            ...
            ScrobbleData scrobbleData = new ScrobbleData(artist.getName(), track.getTitle(),
                    (int) (dateList.get(i).getTime() / 1000));
    ...
    public void importTrackPlayCount(User user) {
        ...
        do {
            ...
            for (Iterator<de.umass.lastfm.Track> it = result.iterator(); it.hasNext();) {
                ...
                log.debug(MessageFormat.format("  Last.fm track name={0} artist={1}", lastFmTrack.getName(),
                ...
                for (TrackDto track : trackDao.findByCriteria(new TrackCriteria()
                    ...
                    log.debug(MessageFormat.format("    Found match in local collection title={0} artistName={1}",
                            track.getTitle(), track.getArtistName()));
                    userTrackDao.initPlayCount(user.getId(), track.getId(), lastFmTrack.getPlaycount()); // Playcount is
                                                                                                         // actually the
                                                                                                         // user play
                                                                                                         // count
                    localCount++;
        ...
        log.info(MessageFormat.format(
                "Retrieved {0} tracks from Last.fm, updated {1} play counts in the local collection", lastFmCount,
                localCount));
    }
    ...
    public void importLovedTrack(User user) {
        ...
        do {
            // XXX implement rate limitation, should be good around 1000*10 = 10k faves for
            // now
            // TODO implement a more permissive track search on local database, if Last.fm
            // corrected the title/artist
            ...
            for (Iterator<de.umass.lastfm.Track> it = result.iterator(); it.hasNext();) {
                lastFmCount++;
                de.umass.lastfm.Track lastFmTrack = it.next();
                log.debug(MessageFormat.format("  Last.fm loved track name={0} artist={1}", lastFmTrack.getName(),
                        lastFmTrack.getArtist()));
                for (TrackDto track : trackDao.findByCriteria(new TrackCriteria()
                        .setTitle(lastFmTrack.getName())
                        .setArtistName(lastFmTrack.getArtist()))) {
        ...
        log.info(MessageFormat.format("Retrieved {0} loved tracks from Last.fm, imported {1} into the local collection",
                lastFmCount, localCount));
    }
    ...
    public String searchTrack(String query) {
        String apiKey = ConfigUtil.getConfigStringValue(ConfigType.LAST_FM_API_KEY);
        System.out.println(apiKey);
        String encodedQuery = URLEncoder.encode(query);
        // Set up the API request URL
        String apiUrl = "http://ws.audioscrobbler.com/2.0/?method=track.search&track=" + encodedQuery + "&api_key="
                + apiKey
                + "&format=json";
        ...
        // Create an HttpGet object with the API request URL
    ...
    public String recommendTracks(String artist, String track) {
        String apiKey = ConfigUtil.getConfigStringValue(ConfigType.LAST_FM_API_KEY);
        System.out.println(apiKey);
        track = URLEncoder.encode(track);
        artist = URLEncoder.encode(artist);
        // Set up the API request URL
        String apiUrl = "http://ws.audioscrobbler.com/2.0/?method=track.getsimilar&artist=" + artist + "&track=" + track
                + "&api_key="
                + apiKey
                + "&format=json";
